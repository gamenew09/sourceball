...
class CSBBot;
class CSBBotCostFunctor;
void Bot_Think(CSBBot *pBot);
...
class CSBBot : public CHL2MP_Player
{
public:
	bool			m_bBackwards;
	...
	float			BotCostFunctor(CNavArea* newArea, CNavArea* area, int t);
};

LINK_ENTITY_TO_CLASS(sb_bot, CSBBot);
...
public:
	static CBasePlayer* ClientPutInServerOverride_Bot(edict_t *pEdict, const char *playername)
	{
		...
		CSBBot *pPlayer = static_cast<CSBBot *>(CreateEntityByName("sb_bot"));
		if (pPlayer)
		{
...
//-----------------------------------------------------------------------------
CBasePlayer *SBBotPutInServer(bool bFrozen)
{
	char botname[64];
	Q_snprintf(botname, sizeof(botname), "Bot%02i", g_CurBotNumber);


	// This trick lets us create a CSBBot for this client instead of the CHL2MP_Player
	...
	CSBBot *pPlayer = ((CSBBot*)CBaseEntity::Instance(pEdict));
...
void SBBot_f()
{
	SBBotPutInServer(false);
}

ConCommand sb_Bot("sb_bot_add", SBBot_f, "Adds a SourceBall bot, different from \"bot\".", FCVAR_CHEAT);
...
void Bot_RunAll(void)
{
	for (int i = 1; i <= gpGlobals->maxClients; i++)
	{
		...
		{
			CSBBot *pBot = dynamic_cast< CSBBot* >(pPlayer);
			if (pBot)
				Bot_Think(pBot);
		}
	}
...
void Bot_UpdateStrafing(CSBBot *pBot, CUserCmd &cmd)
{
...
void Bot_UpdateDirection(CSBBot *pBot)
{
...
void Bot_FlipOut(CSBBot *pBot, CUserCmd &cmd)
{
...
void Bot_HandleSendCmd(CSBBot *pBot)
{
...
void Bot_ForceFireWeapon(CSBBot *pBot, CUserCmd &cmd)
{
...
void Bot_SetForwardMovement(CSBBot *pBot, CUserCmd &cmd)
{
...
void Bot_HandleRespawn(CSBBot *pBot, CUserCmd &cmd)
{
...
/*
Purpose: Get's the nearest Nav Area to a specified Vector.
CNavArea* GetNearestNavAreaToVector(Vector vector)
{
	// Variables for distance and the final nav area.
	float minDist = 0;
	CNavArea* foundArea;

	// Iterate through each nav area and get distance from the passed vector.
	for (int i = 0; i < TheNavAreas.Size(); i++)
	{
		// Get nav area using array operator []
		CNavArea* nArea = TheNavAreas[i];
		// Get distance using Vector.DistTo
		vec_t dist = nArea->GetCenter().DistTo(vector);
		// Check if vec_t(float) is above minDist.
		if (dist > minDist)
		{
			// If so, set minDist to dist and foundArea to the area we found.
			minDist = dist;
			foundArea = nArea;
		}
	}

	// Once done iterating we just return nav area.
	return foundArea;
}
*/

/*
void Bot_NavPath(CSBBot *pBot)
{
	CNavArea* startGoal = GetNearestNavAreaToVector(pBot->GetAbsOrigin());
	if (startGoal)
	{
		//TheNavAreas.Random()
		//float travelDist = NavAreaTravelDistance<CSBBot::BotCostFunctor>(startGoal, startGoal, &CSBBot::BotCostFunctor);
	}
}
*/
